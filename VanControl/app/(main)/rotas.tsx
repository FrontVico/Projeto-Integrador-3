...
import RotasAdminScreen from './rotasAdmin';
import RotasPassageiroScreen from './rotasPassageiro';
...
export default function RotasScreen() {
  ...
  if (user.role === 'PASSAGEIRO') {
    return <RotasPassageiroScreen user={user} />;
  }

  if (user.role === 'ADMIN' || user.role === 'MOTORISTA') {
    return <RotasAdminScreen user={user} />;
  }
